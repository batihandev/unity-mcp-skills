---
name: unity-test
description: "Use when users want to run, list, or check test results."
---
...
**Routing**:
- For compile error checking → use `editor_get_state` (`isCompiling` field)
- For test script creation → `test_create_editmode` / `test_create_playmode`, then modify via `script` module
- For broad regression probes across many skills → `test_smoke_skills`, which uses transient probes to avoid side effects
